...
ECharts 误差图扩展，可在柱状图、条形图、折线图和点阵/散点图上绘制上/下误差区间。导入本包即可注册 `series.type = 'errorChart'`。
...
| `variant` | 选择与误差区间一起绘制的图形形态。`dot`、`point` 和 `marker` 都是二维点阵误差布局的别名。 | `'column' \| 'bar' \| 'line' \| 'scatter' \| 'marker' \| 'dot' \| 'point'` |
| `data` | 误差图数据。分类形态读取 category、value、low/high 或 lowerError/upperError；点阵/散点形态还会读取 x 和 x 方向误差字段。 | `Array<Object \| Array>` |
...
| `xField` | 点阵/散点形态使用的 x 数值字段。 | `string \| number` |
| `yField` | 点阵/散点形态使用的 y 数值字段。 | `string \| number` |
| `xLowField` | 点阵/散点形态的绝对 x 下界字段。 | `string \| number` |
| `xHighField` | 点阵/散点形态的绝对 x 上界字段。 | `string \| number` |
| `xMinusField` | 相对 x 下误差值字段。 | `string \| number` |
| `xPlusField` | 相对 x 上误差值字段。 | `string \| number` |
| `yLowField` | 点阵/散点形态的绝对 y 下界字段。 | `string \| number` |
| `yHighField` | 点阵/散点形态的绝对 y 上界字段。 | `string \| number` |
