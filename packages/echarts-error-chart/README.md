...
ECharts extension chart for drawing upper and lower error intervals on column, bar, line, and dot/scatter charts. Import this package for side effects to register `series.type = 'errorChart'`.
...
Dot point variants can draw horizontal and vertical error bars on two numeric axes:
...
| `variant` | Chooses the mark shape used with the error intervals. `dot`, `point`, and `marker` are aliases for the cartesian point-error layout. | `'column' \| 'bar' \| 'line' \| 'scatter' \| 'marker' \| 'dot' \| 'point'` |
| `data` | Error chart rows. Category variants read category, value, low/high or lowerError/upperError. Dot/scatter variants also read x and x error fields. | `Array<Object \| Array>` |
...
| `xField` | Numeric x field for dot/scatter variants. | `string \| number` |
| `yField` | Numeric y field for dot/scatter variants. | `string \| number` |
| `xLowField` | Absolute x lower bound for dot/scatter variants. | `string \| number` |
| `xHighField` | Absolute x upper bound for dot/scatter variants. | `string \| number` |
| `xMinusField` | Relative x lower error magnitude. | `string \| number` |
| `xPlusField` | Relative x upper error magnitude. | `string \| number` |
| `yLowField` | Absolute y lower bound for dot/scatter variants. | `string \| number` |
| `yHighField` | Absolute y upper bound for dot/scatter variants. | `string \| number` |
...
| `xMin` | Lower bound for the dot/scatter x axis. | `number` |
| `xMax` | Upper bound for the dot/scatter x axis. | `number` |
...
| `symbolSize` | Circle size for line and dot/scatter variants. | `number` |
| `valueAxis` | Styles the value axis, labels, split lines, and axis name. | `Object` |
| `xAxis` | Styles the numeric x axis for dot/scatter variants. | `Object` |
