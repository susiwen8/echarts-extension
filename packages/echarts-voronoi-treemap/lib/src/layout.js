...
export function resolveVoronoiTreemapLayout(option = {}) {
...
export function layoutVoronoiTreemap(data, options = {}) {
...
export function flattenVoronoiTreemapData(data, options = {}) {
...
function computeValues(node, options) {
    ...
    const explicitValue = readNonNegativeNumber(readField(node.raw, options.valueField ?? 'value', options.dimensions, 1, ['size', 'weight', 'amount']));
...
function layoutChildren(node, options) {
    if (!node.children.length || node.points.length < 3 || polygonArea(node.points) <= EPSILON)
        return;
...
function partitionWeightedVoronoi(container, children, maxIteration) {
    ...
    return createPowerCells(container, sites).map((cell, index) => {
        if (cell.points.length >= 3 && polygonArea(cell.points) > EPSILON)
            return cell;
        return {
...
function readChildren(raw, childrenField = 'children') {
    ...
    if (Array.isArray(value))
        return value;
    if (childrenField !== 'children' && Array.isArray(raw.children))
        return raw.children;
    return [];
}
...
function readItemColor(raw) {
    if (!isPlainObject(raw) || !isPlainObject(raw.itemStyle))
        return undefined;
...
function normalizeColors(value) {
    return Array.isArray(value) ? value.filter((color) => typeof color === 'string') : undefined;
}
function normalizeDimensions(value) {
    return Array.isArray(value) ? value.filter((dimension) => typeof dimension === 'string') : undefined;
}
