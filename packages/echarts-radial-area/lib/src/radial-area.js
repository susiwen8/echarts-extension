...
function drawLine(echartsInstance, group, seriesModel, layout) {
    if (layout.valuePolygon.length < 2)
        return;
    ...
    if (!lineStyle.stroke || finiteNumber(lineStyle.lineWidth, 1) <= 0 || finiteNumber(lineStyle.opacity, 1) <= 0)
        return;
...
function pointsToTuples(points, closed) {
    ...
    if (closed && tuples.length > 2)
        tuples.push(tuples[0]);
    return tuples;
}
...
//# sourceMappingURL=radial-area.js.map