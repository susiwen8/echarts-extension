...
![Lollipop 图表截图](../../visual-baseline/echarts-lollipop.png)
...
| 配置项 | 说明 | 可选值 |
| --- | --- | --- |
| `type` | 向 ECharts 注册该包的系列类型。 | `'lollipop'` |
...
| `data` | 分类-数值 记录 drawn as stems and 符号。 | `数组<对象 \| 未知[]>` |
...
| `min` | 数值轴手动最小值。 | `数字` |
| `max` | 数值轴手动最大值。 | `数字` |
| `baseline` | 数值 where stems begin。 | `数字` |
| `tickCount` | 数值轴首选刻度数量。 | `数字` |
| `nice` | Rounds 数值 extent to nicer tick 数值。 | `布尔值` |
| `large` | Enables large-data 渲染 path。 | `布尔值` |
| `symbolSize` | 大小 of lollipop 符号。 | `数字` |
| `grid` | 显示或隐藏图表 网格。 | `对象` |
...
| `stemStyle` | 设置棒棒糖图竖线样式。 | `对象` |
| `stemStyle.color` | 主颜色。 | `字符串` |
| `stemStyle.stroke` | 描边颜色。 | `字符串` |
| `stemStyle.width` | 宽度值。 | `数字` |
| `stemStyle.lineWidth` | 线宽。 | `数字` |
| `stemStyle.opacity` | 透明度。 | `数字` |
| `stemStyle.type` | 线条或图元类型。 | `'solid' \| 'dashed' \| 'dotted' \| 数字[]` |
| `itemStyle` | 设置棒棒糖图符号样式。 | `对象` |
