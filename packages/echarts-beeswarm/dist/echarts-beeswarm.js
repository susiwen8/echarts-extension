(function(global, factory) {
	...
	var DEFAULT_SYMBOL_SIZE = 12;
	var DEFAULT_COLLISION_PADDING = 1;
	var EPSILON = 1e-9;
	function resolveBeeswarmLayout(option = {}) {
		...
		const merged = {
			...
			symbolSize: finiteNumber$1(option.symbolSize, finiteNumber$1(layoutOptions.symbolSize, finiteNumber$1(layout.symbolSize, void 0))),
			collisionPadding: finiteNumber$1(option.collisionPadding, finiteNumber$1(layoutOptions.collisionPadding, finiteNumber$1(layout.collisionPadding, void 0))),
			swarmRadius: finiteNumber$1(option.swarmRadius, finiteNumber$1(layoutOptions.swarmRadius, finiteNumber$1(layout.swarmRadius, void 0)))
		};
		return layoutBeeswarm(Array.isArray(option.data) ? option.data : [], merged);
	}
	function layoutBeeswarm(data, options = {}) {
		...
		return {
			...
			categories,
			valueExtent,
			ticks: createTicks(valueExtent.min, valueExtent.max, tickCount).map((value) => createTick(value, valueExtent, plot, orient)),
			categoryLabels: categories.map((category, index) => createCategoryLabel(category, index, categories.length, plot, orient)),
			points: layoutPoints(ordered, categories, valueExtent, plot, orient, options)
		};
	}
	function normalizeItems(data, options) {
		const dimensions = normalizeDimensions(options.dimensions);
		const defaultRadius = Math.max(0, finiteNumber$1(options.symbolSize, DEFAULT_SYMBOL_SIZE)) / 2;
		const normalized = [];
		data.forEach((item, dataIndex) => {
			...
			const itemSymbolSize = finiteNumber$1(record.symbolSize, defaultRadius * 2);
			normalized.push({
	...
	function layoutPoints(items, categories, extent, plot, orient, options) {
		...
		const result = [];
		const categoryBand = orient === "vertical" ? plot.width / Math.max(categories.length, 1) : plot.height / Math.max(categories.length, 1);
		const defaultSwarmRadius = Math.max(0, categoryBand * .44 - maxRadius(items));
		const swarmRadius = Math.max(0, finiteNumber$1(options.swarmRadius, defaultSwarmRadius));
		const collisionPadding = Math.max(0, finiteNumber$1(options.collisionPadding, DEFAULT_COLLISION_PADDING));
		categories.forEach((category, categoryIndex) => {
			const group = (byCategory.get(category) || []).slice().sort((left, right) => left.value - right.value || left.dataIndex - right.dataIndex);
			const placed = [];
			const categoryCoord = projectCategory(categoryIndex, categories.length, plot, orient);
			group.forEach((item) => {
				const valueCoord = orient === "vertical" ? projectValueToY(item.value, extent, plot) : projectValueToX(item.value, extent, plot);
	...
	function projectCategory(index, count, plot, orient) {
		...
		return start + span * ((index + .5) / count);
	}
	...
	function readOrient(value) {
		return value === "vertical" || value === "horizontal" ? value : void 0;
	}
	...
	echartsHost.extendChartView({
		...
		render(seriesModel, ecModel, api) {
			...
			try {
				...
				if (typeof console !== "undefined") console.error("[beeswarm] render failed", error);
			}
		},
		remove() {
	...
	function readPointStyle(data, seriesModel, itemModel, point, pointIndex) {
		...
		return {
			fill: itemStyleModel.get("color") || visualStyle.fill || seriesItemStyleModel.get("color") || DEFAULT_COLORS[pointIndex % DEFAULT_COLORS.length],
			...
			opacity: finiteNumber(itemStyleModel.get("opacity"), finiteNumber(seriesItemStyleModel.get("opacity"), .88)),
...
//# sourceMappingURL=echarts-beeswarm.js.map