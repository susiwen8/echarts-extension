...
Set `fluid.enabled` to evolve packed nodes over time with Evolution Fluid-style merge, split, and move events. Completed merge/acquire events remove source nodes, transfer their values into the target, and repack the hierarchy. Split/spinOff events reveal pending target nodes from their source, and move/relocate/transfer events carry an existing node from one parent to another. Active events interpolate between the before/after packings and render liquid surfaces between the related circles. Use `fluid.renderMode: 'evolutionFluid'` when you want the packed circles to be drawn as droplets rather than bordered nested circles.
...
| `fluid` | Enables Evolution Fluid-style timed hierarchy evolution. | `Object` |
| `fluid.enabled` | Enables fluid event processing when true. | `boolean` |
| `fluid.renderMode` | Uses the normal circle-packing renderer or the Evolution Fluid droplet renderer for active fluid stories. | `'circlePacking' \| 'evolutionFluid'` |
| `fluid.currentTime` | Current event playback time. | `string \| number \| Date \| null` |
| `fluid.events` | Merge, acquire, split, spin-off, move, relocate, transfer, or checkpoint events between circle nodes. | `Array<object>` |
| `fluid.events.time` | Event time or discrete step. | `string \| number \| Date` |
| `fluid.events.type` | Event type. | `'merge' \| 'acquire' \| 'split' \| 'spinOff' \| 'move' \| 'relocate' \| 'transfer' \| 'checkpoint' \| string` |
| `fluid.events.sources` | Source node ids, raw ids, or names. | `Array<string \| number>` |
| `fluid.events.source` | Single source node id, raw id, name, or source array alias. | `string \| number \| Array<string \| number>` |
| `fluid.events.from` | Alias for `source` / `sources`. | `string \| number \| Array<string \| number>` |
| `fluid.events.targets` | Target node ids, raw ids, or names. | `Array<string \| number>` |
| `fluid.events.target` | Single target node id, raw id, name, or target array alias. | `string \| number \| Array<string \| number>` |
| `fluid.events.to` | Alias for `target` / `targets`. | `string \| number \| Array<string \| number>` |
| `fluid.events.value` | Optional event scale metadata. | `number \| string` |
| `fluid.events.duration` | Optional duration/span for the event in the same units as `time`. | `number` |
| `fluid.events.span` | Alias for `duration`. | `number` |
| `fluid.events.bridge` | Set to false to apply the event without drawing a liquid bridge. | `boolean` |
| `fluid.events.showBridge` | Alias for `bridge`. | `boolean` |
| `fluid.events.drawBridge` | Alias for `bridge`. | `boolean` |
| `fluid.bridgeOpacity` | Opacity for active liquid bridges. | `number` |
| `fluid.bridgeThreshold` | Maximum preferred distance for bridge rendering. | `number` |
| `fluid.bridgeColor` | Bridge fill color override. | `string` |
