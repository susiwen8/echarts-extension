...
![Concentric 图表截图](../../visual-baseline/echarts-concentric.png)
...
| 配置项 | 说明 | 可选值 |
| --- | --- | --- |
| `type` | 向 ECharts 注册该包的系列类型。 | `'concentric'` |
...
| `layout.sortBy` | 对节点 before assigning 同心 levels排序分级。与 sortBy 效果相同。 | `字符串 \| 函数; 默认值 'degree'` |
| `layout.maxLevelDiff` | 最大值 rank difference before 开始ing a new 环。与 maxLevelDiff 效果相同。 | `数字` |
| `layout.preventOverlap` | 扩展环 so 节点 do not overlap。与 preventOverlap 效果相同。 | `布尔值` |
| `layout.equidistant` | 强制every 环 间距 to be equal。与 equidistant 效果相同。 | `布尔值` |
| `layout.sweep` | 用于each 环的角度 sweep。与 sweep 效果相同。 | `数字 (radians)` |
| `layout.startAngle` | 开始ing 角度 for the first 节点 on a 环。与 startAngle 效果相同。 | `数字 (radians)` |
| `layout.clockwise` | 放置节点 顺时针。与 clockwise 效果相同。 | `布尔值` |
| `layout.nodeSize` | 环间距使用的节点大小。与 nodeSize 效果相同。 | `数字 \| 数字[] \| 函数` |
| `layout.nodeSpacing` | 环间距使用的Extra 节点 间距。与 nodeSpacing 效果相同。 | `数字 \| 函数` |
| `sortBy` | 与 layout.sortBy 效果相同。对节点 before assigning 同心 levels排序分级。 | `字符串 \| 函数; 默认值 'degree'` |
| `maxLevelDiff` | 与 layout.maxLevelDiff 效果相同。最大值 rank difference before 开始ing a new 环。 | `数字` |
| `preventOverlap` | 与 layout.preventOverlap 效果相同。扩展环 so 节点 do not overlap。 | `布尔值` |
| `equidistant` | 与 layout.equidistant 效果相同。强制every 环 间距 to be equal。 | `布尔值` |
| `sweep` | 与 layout.sweep 效果相同。用于each 环的角度 sweep。 | `数字 (radians)` |
| `startAngle` | 与 layout.startAngle 效果相同。开始ing 角度 for the first 节点 on a 环。 | `数字 (radians)` |
| `clockwise` | 与 layout.clockwise 效果相同。放置节点 顺时针。 | `布尔值` |
| `nodeSize` | 与 layout.nodeSize 效果相同。环间距使用的节点大小。 | `数字 \| 数字[] \| 函数` |
| `nodeSpacing` | 与 layout.nodeSpacing 效果相同。环间距使用的Extra 节点 间距。 | `数字 \| 函数` |
