...
import type { GraphData, LayoutGraph, LayoutOptions, LayoutResult, Point } from './types.js';

export function computeMDSLayout(input: GraphData, options: LayoutOptions = {}): LayoutResult {
  ...
  const linkDistance =
    typeof configuredLinkDistance === 'number' && Number.isFinite(configuredLinkDistance)
      ? configuredLinkDistance
      : 50;
  const distances = replaceInfinity(allPairsShortestPaths(graph)).map((row) =>
    row.map((value) => value * linkDistance)
  );
...
function preventMDSOverlap(graph: LayoutGraph, center: Point, options: LayoutOptions): void {
...
function deterministicPairAngle(leftIndex: number, rightIndex: number, count: number): number {
  const seed = ((leftIndex + 1) * 73856093) ^ ((rightIndex + 1) * 19349663) ^ (count * 83492791);
  return ((seed >>> 0) % 3600) / 3600 * Math.PI * 2;
}

function recenterNodes(graph: LayoutGraph, center: Point): void {
...
export function runMDS(distances: number[][], dimension = 2, fallbackDistance = 50): number[][] {
  ...
  const matrix = squared.map((row, i) =>
...
function fallbackCircle(n: number, distance: number): number[][] {
