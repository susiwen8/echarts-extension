...
export interface VectorFieldDataPoint {
  dataIndex: number;
  x: number;
  y: number;
  u: number;
...
export interface VectorFieldLayoutItem extends VectorFieldDataPoint {
  screenU: number;
  screenV: number;
  angle: number;
  length: number;
...
export interface VectorFieldLayoutOptions {
  width?: number;
  height?: number;
  padding?: number;
...
export interface VectorFieldLayoutOption extends VectorFieldLayoutOptions {
  data?: unknown;
  layout?: unknown;
  layoutOptions?: unknown;
}

export interface VectorFieldLayoutResult {
  width: number;
  height: number;
  padding: number;
  xExtent: [number, number];
  ...
  invertY: boolean;
  maxMagnitude: number;
  items: VectorFieldLayoutItem[];
}

export function resolveVectorFieldLayout(option: VectorFieldLayoutOption = {}): VectorFieldLayoutResult {
...
export function layoutVectorField(data: unknown, options: VectorFieldLayoutOptions = {}): VectorFieldLayoutResult {
  ...
  const points = samplePoints(
    normalizeVectorFieldData(data, options),
    Math.max(1, Math.floor(finiteNumber(options.samplingStep, 1)))
  );
  ...
  const items = points.map((point) => {
    ...
    const y = mapLinear(
      point.y,
      yExtent,
      invertY ? padding + innerHeight : padding,
      invertY ? padding : padding + innerHeight
    );
...
export function normalizeVectorFieldData(data: unknown, options: VectorFieldLayoutOptions = {}): VectorFieldDataPoint[] {
  if (!Array.isArray(data)) return [];

  const points: VectorFieldDataPoint[] = [];
...
function samplePoints(points: VectorFieldDataPoint[], samplingStep: number): VectorFieldDataPoint[] {
  if (samplingStep <= 1) return points;
  return points.filter((point, index) => index % samplingStep === 0 || point.magnitude === 0);
}

function inferDefaultMaxLength(
  points: VectorFieldDataPoint[],
  xExtent: [number, number],
  yExtent: [number, number],
  innerWidth: number,
  innerHeight: number
  ...
  if (screenSteps.length) return Math.max(4, Math.min(...screenSteps) * 0.78);
  return Math.max(6, Math.min(innerWidth, innerHeight) * 0.05);
}

function minPositiveStep(values: number[]): number | null {
...
function normalizeExtent(value: [number, number]): [number, number] {
...
function mapLinear(value: number, domain: [number, number], rangeStart: number, rangeEnd: number): number {
...
function readExtent(value: unknown): [number, number] | undefined {
...
function finiteNumber(value: unknown, fallback: number | undefined): number {
  return typeof value === 'number' && Number.isFinite(value) ? value : fallback as number;
}
...
function readName(raw: unknown, dataIndex: number): string {
