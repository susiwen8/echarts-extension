...
export function resolveSubwayLayout(option = {}) {
    ...
    const colors = Array.isArray(option.colors) ? option.colors.filter((color) => typeof color === 'string') : undefined;
    ...
    return layoutSubway(routes, {
        ...
        stationRadius: finiteNumber(option.stationRadius, finiteNumber(layoutOptions.stationRadius, finiteNumber(layout.stationRadius, DEFAULT_STATION_RADIUS))),
        interchangeRadius: finiteNumber(option.interchangeRadius, finiteNumber(layoutOptions.interchangeRadius, finiteNumber(layout.interchangeRadius, DEFAULT_INTERCHANGE_RADIUS))),
...
export function layoutSubway(routesInput, options = {}) {
...
export function collectSubwayStationData(routesInput) {
    const stations = [];
    const seen = new Set();
...
function normalizeRoutes(routesInput) {
    return routesInput
        ...
        .filter((route) => route != null && (route.stations.length > 0 || route.points.length > 0));
}
...
function mergeStations(routes) {
    ...
    routes.forEach((route) => {
        route.stations.forEach((station) => {
            ...
            if (!existing.lines.includes(route.id))
                existing.lines.push(route.id);
            if (!existing.labelPosition && station.labelPosition)
                existing.labelPosition = station.labelPosition;
