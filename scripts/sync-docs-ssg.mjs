...
function localizeChineseText(html) {
  const replacements = new Map([
    ...
    ['Sorting algorithm frames are rendered as bars with compare, swap, write, pivot, and sorted states.', '排序算法帧以柱形呈现比较、交换、写入、枢轴和已排序状态。'],
    ['Shortest path search frames show visited nodes, inspected edges, relaxations, tentative distances, and the final path.', '最短路径搜索帧展示已访问节点、检查边、松弛边、暂定距离和最终路径。'],
