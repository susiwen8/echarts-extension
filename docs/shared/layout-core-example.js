...
if (host) {
  cases.forEach((layoutCase) => {
    const existingCard = host.querySelector(`#layout-${layoutCase.id}`);
    const card = existingCard || createLayoutCardElement(layoutCase);
    if (!existingCard) host.append(card);
...
function createLayoutCardElement(layoutCase) {
  ...
  card.innerHTML = createLayoutCardInnerMarkup(layoutCase);
  card.id = `layout-${layoutCase.id}`;
  card.className = 'layout-card';
  card.dataset.layoutCase = layoutCase.id;
  return card;
}
